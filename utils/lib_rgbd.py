...
def vis_rgbd(d_img_path, rgb_img_path=None, save_path="depth_visualization.png"):
    ...
    if rgb_img_path is not None:
        ...
        visualized_depth = cv2.addWeighted(
            visualized_depth, alpha, rgb_img, 1 - alpha, 0
        )
...
def vis_d(d_img_path, save_path, show=False):
    ...
    output_displayable = cv2.normalize(
        depth_image, None, 255, 0, cv2.NORM_MINMAX, cv2.CV_8U
    )
