...
class GEMINI(object):
    ...
    def text_img_to_text(
        self, text, img_path=None, if_p=False
    ):  # can a lot of images, like: [text,img1,img2,img3 ...]
        img = Image.open(img_path)
        response = self.gemini_api([text, img], if_p=if_p)
        return response

    def text_imgs_to_text(
        self, text, img_paths=None, if_p=False
    ):  # can a lot of images, like: [text,img1,img2,img3 ...]
        prompt = [text]
        if img_paths:
    ...
    def text_audio_to_text(
        self, text, audio_path, if_p=False
    ):  # WAV/MP3/AIFF/AAC/OGG/FLAC
        audio_file = self.upload_file(audio_path)
        response = self.gemini_api(
            [text, audio_file], if_p=if_p
        )  # "Provide a transcript of the speech from 02:30 to 03:29."
        return response

    def text_video_to_text(
        self, text, video_path, if_p=False
    ...
    def gemini_detection(
        ...
        result = (
            1 if result_probs[1] > result_probs[0] else 0
        )  # Reward is 1 if closer to 'open door' prompt, 0 otherwise
        result_text = text_prompt[result]

        if if_p:
            # print("result:", result)  # prints: 1/0
            print(
                f"[Gemini INFO] Result: {result_text}\t Label probs: {result_probs}"
            )  # door is open/closed
...
if __name__ == "__main__":
    ...
    gemini.text_imgs_to_text(
        ll_prompt, img_paths=[example_img1, example_img2, img_path], if_p=True
    )
