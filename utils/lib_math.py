...
def EulerAngle_to_R(rpy, rad=False):
    ...
    if rad:
        ...
        R = tfs.euler.euler2mat(
            np.radians(r), np.radians(p), np.radians(y), axes="sxyz"
        )
    return R
...
def rotate_R(R, delta_r=0, delta_p=0, delta_y=0, rotate_order="zyx", rad=False):
    ...
    R_p = np.array(
        [
            [np.cos(delta_p), 0, np.sin(delta_p)],
            [0, 1, 0],
    ...
    R_y = np.array(
        [
            [np.cos(delta_y), -np.sin(delta_y), 0],
...
def rotate_xyzrpy(
    xyzrpy, delta_r=0, delta_p=0, delta_y=0, rotate_order="zyx", rad=False
):
    xyzrpy = np.array(xyzrpy)
...
def test1():
    R = [
        ...
        [-0.9973485867817671, 0.06353005346138718, 0.03549265771403007],
        [-0.033265015138603936, 0.03578547611006781, -0.9988057060647002],
    ]
    R = [
        [-0.99978544, 0.01618313, 0.01292909],
        [0.01924667, 0.95650893, 0.29106747],
        [-0.00765641, 0.29125386, -0.95661516],
    ]
...
def test2():
    cam2baselink_H = xyzrpy_to_H(
        np.array(
            [0.04259999, 0.02724008, 1.234697, -111.8845794, 1.55444518, -92.16392625]
        )
    )
    print(f"cam2baselink_H:\n{cam2baselink_H}")
    rlink12baselink_H = xyzrpy_to_H(
        np.array([-0.0096088, -0.17189, 1.1246, 90, 0, -64.9724037791713])
    )
    print(f"rlink12baselink_H:\n{rlink12baselink_H}")
    cam2rlink1_H = cam2baselink_H @ np.linalg.inv(
        rlink12baselink_H
    )  # cam2baselink_H @ rlink12baselink_H
    print(f"cam2rlink1_H:\n{cam2rlink1_H}")


def test3():
    rubik_center = [0.46196093592231313, 0.16167266059444962, -0.516, 1]
    r_cam2base_H = np.array(
        [
            [0.01155362, 0.84610748, 0.53288708, -0.1165359],
            [-0.4203419, -0.47943922, 0.77035753, 0.14733703],
            [0.90729224, -0.23289519, 0.35011516, -0.02538692],
            [0, 0, 0, 1],
        ]
    )
...
def test4():
    R = [
        [-0.3220086, -0.19165227, 0.9271353],
        [-0.91081784, 0.32990057, -0.24814607],
        [-0.2583047, -0.92435655, -0.28079113],
    ]
...
def test6():
    target2cam_xyzrpy = [
        ...
        0.5439752324490061,
        0.3631291403893837,
        -0.008841798828976621,
        -0.003359646326433152,
    ]
