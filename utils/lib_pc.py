...
class PointCloud(object):
    ...
    def upsample_pc(self, upsampling_factor=2, if_p=False):
        ...
        pcd_upsampled.points = o3d.utility.Vector3dVector(
            np.concatenate(new_points, axis=0)
        )
        pcd_upsampled.colors = o3d.utility.Vector3dVector(
            np.concatenate(new_colors, axis=0)
        )
        if if_p:
            print(f"[points num before] {len(self.pcd.points)}")
    ...
    def gen_pc_from_d(self, d_img, pcd_path=None):
        ...
        )  # depth_trunc=1000.0, # Optional: Set a maximum depth for clipping
    ...
    def gen_pc_from_rgbd(self, rgb_img, d_img, pcd_path=None):
        ...
        self.pcd = o3d.geometry.PointCloud.create_from_rgbd_image(
            self.rgbd, intrinsic=self.pinhole_intrinsic, extrinsic=self.extrinsic
        )
