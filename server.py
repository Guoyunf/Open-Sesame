...
class Server():
    def __init__(self,hostname,username,password, port = 22, if_stfp=True):
    ...
    def init_from_yaml(cls,cfg_path='cfg/cfg_server.yaml'):
        ...
        return cls(cfg.hostname,cfg.username,cfg.password, cfg.port, cfg.if_stfp)

    def __str__(self):
        return f'[Server]: hostname: {self.hostname}, port: {self.port}, username: {self.username}, password: {self.password}, if_stfp: {self.if_stfp}'
        ...
        if show_process:
            ...
            with open(local_file_path, 'rb') as f:
                with tqdm(total=file_size, unit='B', unit_scale=True, unit_divisor=1024, desc=f"Upload {os.path.basename(local_file_path)}") as pbar: 
                    ...
                    self.sftp.put(local_file_path, remote_file_path, callback=callback) 
