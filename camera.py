...
class Camera(object):
    def __init__(
        ...
        self.pinhole_intrinsic = o3d.camera.PinholeCameraIntrinsic(
            self.width,
            self.height,
            self.intrinsic.fx,
            self.intrinsic.fy,
    ...
    def get_depth_roi(
        self, u, v, d_img, radius=15, depth_threshold=0.05, valid_ratio_threshold=0.50
    ):
        if isinstance(d_img, str):
            d_img = cv2.imread(d_img, cv2.IMREAD_UNCHANGED)
        ...
        center_depth = np.mean(
            depth_roi[depth_roi != 0]
        )  # center_depth = depth_roi[radius, radius]
        valid_depth_mask = (
            np.abs(depth_roi - center_depth) * self.depth_scale <= depth_threshold
        ) & (depth_roi != 0)
        ...
        if valid_ratio < valid_ratio_threshold:
            print(
                f"ERROR: Not enough valid depth values around the point. Ratio: {valid_ratio:.2f}"
            )
            return None
    ...
    def vis_pc_realtime(self, pcd_path="pc.pcd", img_path="pc.png"):
        ...
        if pcd_path:
            o3d.io.write_point_cloud(
                pcd_path, pcd
            )  # Save as .ply (you can change the format)
    ...
    def check_rs_resolution(self):
        ...
        for profile in depth_sensor.get_stream_profiles():
            if profile.stream_type() == rs.stream.depth:
                width, height = (
                    profile.as_video_stream_profile().width(),
        ...
        try:
            while True:
                ...
                if keyboard.is_pressed("r"):
                    out = cv2.VideoWriter(
                        "realsense_clip.avi", fourcc, 30.0, (1280, 720)
                    )
                    recording = True
